...
if __name__ == "__main__":
    ...
    if mode == "predict":
        ...
        IMAGE_OUTPUT_PATH='./imgss'
        files=os.listdir(IMAGE_INPUT_PATH)
        if not os.path.exists("./abc/"):
            os.makedirs("./abc/")
        for file in files:

            image_input = IMAGE_INPUT_PATH + '/' + file
            image = Image.open(image_input )
            r_image = yolo.detect_image2(image)
            image_output_fullname = IMAGE_OUTPUT_PATH + "/"  + file
            r_image.save(image_output_fullname)
        liebiao=[]
        f1 = open('./abc/data.txt' , 'r')

        for line in f1.readlines():
            line2 = line[1:-2]  # 读取每行去掉前15位的数

            liebiao.append(line2)
        liebiao = [x.strip() for x in liebiao]
        #print(liebiao)
        n=len(liebiao)
        #print(liebiao[0])
        srate=0
        for i in range(n):
            ...
            full=int(a[0])
            empty = int(a[1])
            half= math.ceil(int(a[2])/2)
            print('No.%d rice:Full,Empty and Half' % (i+1), full, empty, 2*half)
            #print(type(a[0]))
            ph = 0.797 * (full / (full + empty)) + 0.1972
            rate = (ph * half + full) / (full + empty + half)
            srate = srate + rate
            print('No.%d Seed setting rate of rice:' % (i+1), rate)
        mrate = srate / n
